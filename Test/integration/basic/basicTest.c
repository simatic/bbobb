...
void myCallbackCircuitChange(circuitView *cp){
  char s[MAX_LEN_ADDRESS_AS_STR];

  printf("!!! ******** myCallbackCircuitChange called with %d members (process ",
      cp->cv_nmemb);
...
int main(int argc, char *argv[]){
  ...
  if (sender) {
    ...
    while (!terminate) {
      message *mp = bbNewmsg(PAYLOAD_SIZE);
      if (mp == NULL ) {
        bbErrorAtLineWithoutErrnum(rc, __FILE__, __LINE__, "newmsg()");
        return EXIT_FAILURE;
      }
      rankMessage++;
      *((int*) (mp->payload)) = rankMessage;
      if (bbOBroadcast(TEST_MESSAGE, mp) < 0) {
        bbErrorAtLineWithoutErrnum(rc, __FILE__, __LINE__, "oBroadcast()");
        return EXIT_FAILURE;
      }
      usleep(delayBetweenTwoOBroadcast);
    }
  } else {
    // Process waits that myCallbackODelivery tells it to die
    do {
